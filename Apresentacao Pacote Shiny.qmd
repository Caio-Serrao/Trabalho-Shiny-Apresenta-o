...
-   gráficos, tabelas, mapas, entre diversas outras saídas;

Aqui que os *labels* do UI receberam suas atribuições.

```{r server, echo=T, eval=F}
...
```{r, renderplotly, echo=TRUE, eval=F}
server <- function(input, output, session){
...
```{r renderDT, echo=TRUE, results='hide'}
server <- function(input, output){
...
1.  **titlePanel() -** título do aplicativo;

2.  **siderbarLayout() -** esse recebe os comandos para posicionamentos de *inputs* e os espaços para os *outputs.*

## Abordando o sidebarLayout()

```{r,siderbarLayout, echo=TRUE}
library(shiny)
